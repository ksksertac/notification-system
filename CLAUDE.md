...
- **ACK Safety Hardening (5 fixes)**:
  ...
  - CAS miss on `UpdateStatusWithDetails(processing→delivered)` now retries: re-reads current status, if non-final forces delivered write. Both error and bool result of retry are checked — second CAS miss also aborts without ACK. Prevents losing delivered record when scheduler concurrently resets status.
  - Max delivery count safety valve: messages redelivered ≥10 times (via XCLAIM) are moved to DLQ and ACKed. If `GetByID` or `MoveToDLQ` fails, message is NOT ACKed — consistent with the no-ACK-on-side-effect-failure principle. `DeliveryCount` field added to `queue.Message`, populated from XPENDING retry count in `ClaimStale`.
- **WebSocket Auth**: `/ws` endpoint moved behind dedicated `WSAPIKeyAuth` middleware that accepts both `X-API-Key` header and `?api_key=` query parameter. REST API (`/api/v1/*`) uses header-only `APIKeyAuth` — query param auth is intentionally excluded from REST routes to prevent API key leakage via access logs, proxy logs, and browser history.
- **Batch Idempotency Replay Fix**: `CreateBatch` now separates replayed (idempotency-hit) notifications from new ones. Only new notifications are published to the stream; replayed ones are included in the response but not re-published. Previously, replayed notifications were added to `immediate` list and re-published, causing duplicate stream messages for already queued/processing notifications.
- **Processing Transition ACK Split**: `UpdateStatus(→processing)` error and CAS miss now have separate handling. CAS miss (another consumer claimed it) → ACK. Redis/write error → no ACK, left for XCLAIM redelivery. Previously both cases ACKed.
